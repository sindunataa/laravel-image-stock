            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('galeries.update',['id'=> $edit->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select type="text" name="album_id" class="form-control" placeholder="album_id" id="album_id">
                                            <option label="Pilih Salah Satu"></option>
                                            @foreach ($album as $albums)
                                            
                                            <option value="{{$albums->id}}" {{($albums->id == $edit->album_id)?"selected":"";}}>{{$albums->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="author_id" class="form-label">Author :</label>
                                        <select type="text" name="author_id" class="form-control" placeholder="author_id" id="author_id">
                                            <option label="Pilih Salah Satu"></option>
                                            @foreach ($author as $authors)
                                            
                                            <option value="{{$authors->id}}" {{($authors->id == $edit->author_id)?"selected":"";}}>{{$authors->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
